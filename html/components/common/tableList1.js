...
class TableLists extends React.Component {
  ...
  getHostList(currentPage, rowsPerPage, searchKey, testGroupId) {
    ...
    let data =
      self.state.startValue && self.state.endValue
        ? {
            group: testGroupId,
  ...
  /**
   * Click the page number, the previ·ous page, the next page when the button operation
   * @param {int} pagenum
   */
  ...
  onChangeInputPage(e) {
    ...
    if (
      hostlistPage != "" &&
      (!re.test(hostlistPage) ||
        hostlistPage == 0 ||
        hostlistPage > this.state.pageCount)
    ) {
  ...
  componentDidMount() {
    ...
    let storage = window.localStorage;
    let currentPage =
      Number(storage.getItem(this.props.searchListApi)) ||
      this.state.currentPage;
    this.getHostList(
      currentPage,
      this.state.rowsPerPage,
    ...
    return (
      <div className="table-list customTableWarp">
        <form onSubmit={this.handleSearchNode.bind(this)} className="clearfix">
          ...
          />
          <button
            className="nodelist-search"
            onClick={this.handleSearchNode.bind(this)}
          ></button>
        </form>
          ...
          <div className="datePicker clearfix">
            <div className="left-pick">
              <div className="select-group">
                <h5 onClick={this.handleToggleTestGroup.bind(this)}>
                  {this.state.testGroup}
                </h5>
                {this.state.toggleTestGroup && (
                  <div
                    className="selevt-list"
                    onClick={this.handleChangeTestGroup.bind(this)}
                  >
                    <p>Group Trias</p>
                    <p>Group Ethereum</p>
        ...
        <table className="customTable">
          <thead>
            {this.props.searchListApi == "/api/node_list/" && (
              <tr>
                <th>
                  <FormattedMessage id="termNode" /> IP
                </th>
          ...
          <tbody>
            {this.props.searchListApi == "/api/node_list/" &&
              this.state.hostlist &&
              this.state.hostlist.map(
                function (i, index) {
                  return (
                    <tr className="" key={index}>
                      <td className="hostIp">{i.node_ip}</td>
                      {/* <td className="">{i.location}</td> */}
                      <td className="hostStatus">
                        <span
                          className={i.status == 0 ? "normal" : "error"}
                        ></span>
                        {i.status == 0 ? "Normal" : "abnormality"}
                      </td>
                      <td className="">{i.block_heigth}</td>
                      <td className="">{i.latest_block_hash}</td>
                      <td className="">
                        {this.getTimeFormat(i.latest_block_time)}
                      </td>
                    </tr>
                  );
                }.bind(this)
                    ...
                    <tr className="" key={index}>
                      {i.group == 0 && (
                        <td className="activity-groupName">
                          <img
                            src={require("../../img/icon/inline/icon_inline_trias@2x.png")}
                            alt=""
                          />
                          Group Trias
                        </td>
                      ...
                      {i.group == 1 && (
                        <td className="activity-groupName">
                          <img
                            src={require("../../img/icon/inline/icon_inline_eth@2x.png")}
                            alt=""
                          />
                          Group Ethereum
                        </td>
                      ...
                      {i.group == 2 && (
                        <td className="activity-groupName">
                          <img
                            src={require("../../img/icon/inline/icon_inline_hyperledger@2x.png")}
                            alt=""
                          />
                          Group Hyperledger
                        </td>
            ...
            {(!this.state.hostlist || this.state.hostlist.length == 0) && (
              <tr
                className=""
                style={{
                  width: "100%",
              ...
              >
                <td style={{ paddingLeft: "40px", width: "100%" }}>
                  当前没有匹配的数据。
                </td>
              </tr>
            )}
        ...
        <CustomPagination
          ...
          rowsPerPageRange={[
            ...
            {
              name: this.props.intl.locale == "zh" ? "100 项/页" : "100 / page",
              value: 100,
            },
            {
              name: this.props.intl.locale == "zh" ? "200 项/页" : "200 / page",
              value: 200,
            },
          ]}
        />
...
export default injectIntl(TableLists);
