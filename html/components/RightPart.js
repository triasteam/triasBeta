...
export default class RightPart extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      JSON.stringify(nextProps.currentInfo) !=
      JSON.stringify(this.props.currentInfo)
    ) {
      this.setState({
      ...
      <div className="right-part">
        <div className="simulations">
          <p className="main-title">
            <FormattedMessage id="titleSimulations" />
          </p>
          <div className="sim-data">
            <p className="clearfix">
              <span className="attr">
                <FormattedMessage id="simuLable1" />
              </span>
            ...
            {this.state.currentEventIndex !== -1 && (
              <p className="clearfix">
                <span className="attr">
                  <FormattedMessage id="simuLable2" />
                </span>
            ...
            {this.state.currentEventIndex == -1 && (
              <p className="clearfix">
                ...
                <span className="value time">00:00:00</span>
              </p>
            )}
            <p className="clearfix">
              <span className="attr">
                <FormattedMessage id="simuLable3" />
              </span>
              <span className="value">
          ...
          <p className="index">
            {this.state.currentInfo.current_index == -1 ? 0 : 1}
            {text}(
            {this.state.currentInfo.all_nodes_num == 0
              ? "0%"
              : ((1 / this.state.currentInfo.all_nodes_num) * 100).toFixed(2) +
                "%"}
            )
          </p>
          <p className="note percent-note">
            <FormattedMessage id="simuLable4" />
          </p>
          <p className="index">
            {this.state.currentInfo.current_index == -1 ||
          ...
          <p className="note">
            <FormattedMessage id="simuLable5" />
          </p>
        </div>
        ...
        <div className="specifications">
          <p className="main-title">
            <FormattedMessage id="titleHardwareSpecifications" />
          </p>
          <section>
