...
class GeneralStatics extends React.Component {
  ...
  getGeneralStatics_bsc() {
    var self = this;
    $.ajax({
      url: "/api/general_static_bsc/",
      type: "GET",
      dataType: "json",
      success: function (data) {
        if (data.status == "success") {
    ...
    return (
      <div className="general-statics">
        <p className="main-title">
          <FormattedMessage id="titleGeneralStatistics" />
        </p>
        <div className="card-group">
          <StaticsCard
            cardInfo={this.state.nodes}
            cardinfo1={this.state.nodes1}
            src={require("../img/icon/general-statics/icon_gs_nodes@2x.png")}
            title={this.state.lang == "zh" ? "节点" : "Nodes"}
          />
          <StaticsCard
            cardInfo={this.state.block_height}
            cardinfo1={this.state.block_height1}
            src={require("../img/icon/general-statics/icon_gs_blockHeight@2x.png")}
            title={this.state.lang == "zh" ? "区块高度" : "Block Height"}
          />
          <StaticsCard
            cardInfo={this.state.accounts}
            cardinfo1={this.state.accounts1}
            src={require("../img/icon/general-statics/icon_gs_account@2x.png")}
            title={this.state.lang == "zh" ? "账户" : "Accounts"}
          />
          {/* <div className="card-container null-card">
            ...
            <section className="parameters">
              {this.state.lang == "zh"
                ? "帐户统计数据将在下一版本中提供。"
                : "Account statistics will be avaliable in the next version."}
            </section>
          </div> */}
          <StaticsCard
            cardInfo={this.state.peak_tx}
            cardinfo1={this.state.peak_tx1}
            src={require("../img/icon/general-statics/icon_gs_peakTx@2x.png")}
            title={this.state.lang == "zh" ? "峰值交易数" : "Peak Tx"}
          />
          <StaticsCard
            cardInfo={this.state.today_tx}
            cardinfo1={this.state.today_tx1}
            src={require("../img/icon/general-statics/icon_gs_txToday@2x.png")}
            title={this.state.lang == "zh" ? "今日交易总数" : "Tx Today"}
          />
          <StaticsCard
            cardInfo={this.state.tx_num}
            cardinfo1={this.state.tx_num1}
            src={require("../img/icon/general-statics/icon_gs_irrTx@2x.png")}
            title={this.state.lang == "zh" ? "不可逆的交易" : "Irreversible Tx"}
          />
        </div>
      </div>
    );
